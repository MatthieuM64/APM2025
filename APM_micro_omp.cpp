...
/*ACTIVE POTTS MODEL - MESTABILITITY OF THE LIQUID STATE - SMALL DIFFUSION*/
...
particle::particle(const int &LX, const int &LY, const int &init)
{
	...
	else if (init==1) //Tranverse band.
	{
		spin=0;
		x=int(3*LX/8+ran()*LX/4);
		y=int(ran()*LY);
	}
	else if (init==2) //Longitudinal band.
	{
		spin=0;
		x=int(ran()*LX);
		y=int(3*LY/8+ran()*LY/4);
	}
	else if (init==3) //Liquid phase.
	{
		spin=1;
		x=int(ran()*LX);
		y=int(ran()*LY);
...
//Export density.
void exportDensity(const vector< vector<int> > &RHO, const double &beta, const double &D, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_APM_dynamics2d/");
	stringstream ssDensity;
	ssDensity << "./data_APM_dynamics2d/APM_density_beta=" << beta << "_D=" << D << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
//Export state.
void exportState(const vector<vector< vector<int> > > &RHOS, const vector< vector<int> > &RHO, const double &beta, const double &D, const double &epsilon, const double &rho0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_APM_dynamics2d/");
	stringstream ssState;
	ssState << "./data_APM_dynamics2d/APM_state_beta=" << beta << "_D=" << D << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
void ReadCommandLine(int argc, char** argv, double &beta, double &D, double &epsilon, double &rho0, int &LX, int &LY, int &tmax, int &init, int &RAN, int &THREAD_NUM)
{
	...
	cout << "-beta=" << beta << "_D=" << D << " -epsilon=" << epsilon << " -rho0=" << rho0 << " -LX=" << LX << " -LY=" << LY << " -tmax=" << tmax << " -init=" << init << " -ran=" << RAN << " -threads=" << THREAD_NUM << endl;
}
...
int main(int argc, char *argv[])
{
	...
	int init=3, tmax=600000, RAN=0, THREAD_NUM=4;

	//Read imported parameters in command line.
	ReadCommandLine(argc,argv,beta,D,epsilon,rho0,LX,LY,tmax,init,RAN,THREAD_NUM);
	...
	ssAverages << "./data_APM_averages/APM_averages_beta=" << beta << "_D=" << D << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		{
			exportDensity(RHO,beta,D,epsilon,rho0,LX,LY,init,RAN,t);
			exportState(RHOS,RHO,beta,D,epsilon,rho0,LX,LY,init,RAN,t);
